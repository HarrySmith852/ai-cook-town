...
```bash
node "${CLAUDE_PLUGIN_ROOT}/codex/scripts/codex-companion.mjs" task-resume-candidate --json
```
...
- The subagent is a thin forwarder only. It should use one `Bash` call to invoke `node "${CLAUDE_PLUGIN_ROOT}/codex/scripts/codex-companion.mjs" task ...` and return that command's stdout as-is.
